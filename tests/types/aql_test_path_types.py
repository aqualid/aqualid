...
class TestPathTypes(AqlTestCase):
    ...
    def test_file_path(self):
        ...
        self.assertEqual(
            p.name(), os.path.splitext(os.path.basename(file1))[0])
        self.assertEqual(p.ext(), os.path.splitext(os.path.basename(file1))[1])
        self.assertIn(
            p.drive(), [os.path.splitdrive(file1)[0], _splitunc(file1)[0]])

        self.assertEqual(p.change(ext='.ttt', dirname='test', name='test_file',
