...
# Original source file was taken from WAF build tool: https://code.google.com/p/waf/source/browse/utils/genbench.py
# Copyright Thomas Nagy, 2005-2014, https://waf.googlecode.com
...
for i in range({libs}):
  ...
  src_files = ['lib_%d/class_%d.cpp' % (i, k) for k in range({classes}) ]
  
  cpp.LinkStaticLibrary( src_files, target = 'lib%s' % i )
"""

def   createAqualidTop( libs, classes ):
    with open('make.aql', 'w') as f:
      content = AQL.format( libs = libs, classes = classes )
      f.write( content )
